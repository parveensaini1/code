...
#if defined(NDEBUG)
# error "Perfectcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4e2e6403694e8dd72fb42c0ea3e9415d6ffb189a959acb7361e0da6ef6f486be");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Perfectcoin: starting difficulty is 1 / 2^12
...
{
    // Perfectcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 60000 blocks, which will occur approximately every 2.3 monthes
    nSubsidy >>= (nHeight / 30000); // Perfectcoin: 60k blocks in ~4 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 49 * 24 * 60 * 60; // Perfectcoin: 24 hours
static const int64 nTargetSpacing = 35*60; // Perfectcoin: 3.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Perfectcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Perfectcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "1505503773";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("06f780e35e6c84d7ee477b6849f1b912daaaa1994968a62f230a31098788424700e18e51c9647a15b70a7b419831bbb328485f7f50ee23833a3113569abcb8ecc3") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1527229930;
        ...
        assert(block.hashMerkleRoot == uint256("0x39e3502b0e2b25cc62ce0a0eac3ab3dd16580a3dfa8219f53b895f8e07281b0b"));
...
unsigned char pchMessageStart[4] = { 0xb5, 0xcb, 0x69, 0x3d }; // Perfectcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PerfectcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PerfectcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PerfectcoinMiner(CWallet *pwallet)
{
    printf("PerfectcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running PerfectcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PerfectcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PerfectcoinMiner, pwallet));
}
